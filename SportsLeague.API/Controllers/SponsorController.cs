...
public class SponsorController : ControllerBase
{
    ...
    public async Task<ActionResult<SponsorResponseDTO>> GetById(int id)
    {
        ...
        if (sponsor == null) return NotFound(new { message = $"Patrocinador con ID {id} no encontrado" });
    ...
    public async Task<ActionResult> LinkToTournament(int id, TournamentSponsorRequestDTO dto)
    {
        try
        {
            ...
            return StatusCode(201, new { message = "Sponsor vinculado al torneo exitosamente" });
        }
        catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
        catch (InvalidOperationException ex) { return Conflict(new { message = ex.Message }); }
    }
